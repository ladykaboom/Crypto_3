...
ten rodzaj szyfrowania - dziêki temu mo¿liwe jest odtwarzanie muzyki od dowolnej sekundy.
...
Program odczytuje zawartosc pliku privatekey.keystore, szyfruje go kluczem k2 (zaszytym w tym programie, linia 56), a nastepnie zapisuje w pliku configEnc.txt
...
Interaktywne wczytywanie has³a odbywa siê za pomoc¹ funkcji _getch()

UWAGA Plik jest przystosowany do szyfrowania/odszyfrowywania plików .mp3. Aby to zmieniæ, nale¿y dodaæ odpowiednie rozszerzenie w linijkach: 253 i 264
...
char *ckey;
unsigned char k2[30] = "ahbsihbvfieyv27323g27328";  //klucz szyfruj¹cy config.txt oraz pass.txt
unsigned char iv[8] ="alamat";
...
void encrypt(char *startFile, char *enrcyptedFile, unsigned char *k2key) {
	...
	*/
	fp = fopen(startFile, "rb");
	encrfp = fopen(enrcyptedFile, "wb");
...
*/
void decrypt(char *encryptedFile, char *dercyptedFile, int sekunda, int file_or_char, unsigned char *k2key) {
	/*
	...
	*/
	encrfp = fopen(encryptedFile, "rb");
	decrfp = fopen(dercyptedFile, "wb");
	if (encrfp == NULL) { fputs("File error", stderr); exit(1); }
